...
class DetectionCellPostProcessor:
    ...
    def _stack_pred_maps(
        ...
        nuclei_type_map_np = cp.asnumpy(nuclei_type_map)
        nuclei_binary_map_np = cp.asnumpy(nuclei_binary_map).squeeze()

        nuclei_binary_map_np = (nuclei_binary_map_np * 255).astype(np.uint8)

        cv2.imshow("nuclei_binary_map_np", nuclei_binary_map_np)
        cv2.waitKey(0)
