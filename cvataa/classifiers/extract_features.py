...
DEFAULT_ANNOTATIONS = (
    "/mnt/NAS/QuPath_Projects_AA/cellpose-dino-cls/annotation_exports/D1/D1_annotations.geojson"
)
...
SHAPE_FEATURES = ["area_um2", "length_um", "circularity", "solidity", "max_diam_um", "min_diam_um"]
...
INTENSITY_FEATURES = [
    f"{s}_{m}" for s in ("hem", "dab") for m in ("mean", "median", "min", "max", "std")
]
...
QUPATH_NAME_MAP = {
    ...
    "solidity": "Solidity",
    "max_diam_um": "Max diameter µm",
    "min_diam_um": "Min diameter µm",
    "hem_mean": "Hematoxylin: Mean",
    "hem_median": "Hematoxylin: Median",
    "hem_min": "Hematoxylin: Min",
    "hem_max": "Hematoxylin: Max",
    "hem_std": "Hematoxylin: Std.Dev.",
    "dab_mean": "DAB: Mean",
    "dab_median": "DAB: Median",
    "dab_min": "DAB: Min",
    "dab_max": "DAB: Max",
    "dab_std": "DAB: Std.Dev.",
}
...
class NucleusFeatureExtractor:
        ...
        for k in ("openslide.mpp-x", "tiffslide.mpp-x", "aperio.MPP"):
            ...
            if v:
                try:
                    ...
                    pass
        raise RuntimeError(
            "Could not read microns-per-pixel from the slide; " "pass mpp explicitly."
        )
    ...
    def read_bbox(self, x0, y0, w, h):
        ...
        arr = np.asarray(self._slide.read_region((x0c, y0c), 0, (x1c - x0c, y1c - y0c)))[..., :3]
        return arr, x0c, y0c
    ...
    def extract_many(self, polygons, centroids=None):
        """Loop over polygons, returning (DataFrame of base features,
        (N,2) array of centroids)."""
        rows, cents = [], []
        t0 = time.time()
        n_polygons = len(polygons)
        for i, poly in enumerate(tqdm(polygons, desc="extract_features", total=n_polygons)):
            pts = np.asarray(poly, dtype=np.float32)
            c = (
                pts.mean(axis=0)
                if centroids is None
                else np.asarray(centroids[i], dtype=np.float32)
            )
...
def smooth_features_single(
    base_df, centroids_um, fwhm_um, prefix, all_centroids_um=None, all_values=None
):
    ...
    if all_centroids_um is not None:
        ...
        for i, idx in enumerate(all_tree.query_ball_point(centroids_um, r=2.0 * fwhm_um)):
    ...
    for i, idx in enumerate(
        tqdm(neighbours, desc=f"smooth_features: {fwhm_um}", total=n_neighbours)
    ):
    ...
    res = pd.DataFrame(out, columns=[f"{prefix}_{c}" for c in BASE_FEATURES], index=base_df.index)
    res[f"{prefix}_nearby_count"] = counts
    return res


def add_smoothed_features(
    base_df, centroids_px, mpp, fwhms=(50.0, 75.0), all_centroids_px=None, all_base_df=None
):
    ...
    for f in fwhms:
        ...
        parts.append(
            smooth_features_single(
                base, centroids_um, f, prefix, all_centroids_um=all_um, all_values=all_vals
            )
        )
...
def assign_labels(centroids_px, annotation_path, verbose=True):
    ...
    for nm, ring, _area, x0, x1, y0, y1 in regions:
        ...
        sub = unassigned
        m = (cx[sub] >= x0) & (cx[sub] <= x1) & (cy[sub] >= y0) & (cy[sub] <= y1)
        cand = sub[m]
        if cand.size == 0:
            continue
...
def main():
    ...
    ap.add_argument(
        "--geojson", default=DEFAULT_GEOJSON, help="Detection polygons (.geojson or .geojson.gz)"
    )
    ap.add_argument(
        "--annotations",
    ...
    if annotations and not os.path.exists(annotations):
        print(f"  WARNING: annotations not found ({annotations}) - " f"continuing without labels")
        annotations = None
    ...
    if annotations:
        ...
        print(f"    {n_lab}/{len(labels)} nuclei inside an annotation " f"({time.time()-t0:.1f}s)")
        ...
        if not args.keep_unlabeled:
            ...
            print(
                f"  extracting features for {len(keep)} labelled nuclei "
    ...
    with NucleusFeatureExtractor(args.wsi, mpp=args.mpp) as ext:
        print(
            f"  {ext.backend}, {ext.dimensions[0]}x{ext.dimensions[1]} px, " f"{ext.mpp:.6f} um/px"
        )
    ...
    if args.min_area_um2 > 0 or np.isfinite(args.max_area_um2):
        keep = (
            (base["area_um2"] >= args.min_area_um2) & (base["area_um2"] <= args.max_area_um2)
        ).to_numpy()
        n_drop = int((~keep).sum())
        if n_drop:
    ...
    df = add_smoothed_features(base, centroids, mpp, all_centroids_px=centroids_every_detection)
    ...
    print(f"\nWrote {len(df)} nuclei x {len(BASE_FEATURES)*3 + 2} features " f"-> {args.out}")
